...
def minkunet_custom_collate_fn(list_data):
    ...
    labelized_panoptic = len(input) >= 8
...
class NuScenesDataset(Dataset):
    ...
    def __init__(self, phase, config, transforms=None, cached_nuscenes=None):
        ...
        if phase in ("val", "verifying", 'test'):
            skip_ratio = 1
        else:
            try:
    ...
    def __getitem__(self, idx):
        ...
        if self.labels:
            ...
            if self.panoptic:
                ...
                panoptic_label = np.load(panoptic_label_filename)['data']
        ...
        if self.labels:
            ...
            unique_labels = points_labels[indexes]  # point -> voxel  0~16
            #
            if self.panoptic:
                point_label_sem = torch.tensor(
                    np.vectorize(self.eval_labels.__getitem__)(panoptic_label // 1000),
                    dtype=torch.int32,
                )
                point_inst_label = panoptic_label

                points_labels = point_label_sem
