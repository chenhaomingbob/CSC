...
kitti_pc_data_file_suffix = 'bin'
kitti_pc_data_directory_name = 'velodyne'
kitti_pc_label_file_suffix = 'label'
kitti_dataset_root_dir = './datasets/semantickitti/raw/dataset/sequences'

semantic_kitti_pc_label_directory_name = 'labels'  # for SemanticKITTI
scribble_kitti_pc_label_directory_name = 'scribbles'  # for ScribbleKITTI
...
class SemanticKITTIDataset(Dataset):
    ...
    def __init__(self, phase, config, transforms=None):
        ...
        self.label_directory = scribble_kitti_pc_label_directory_name \
            if config.get('use_scribble_kitti', False) and self.phase == 'train' \
            else semantic_kitti_pc_label_directory_name
        ##################################################################
        ...
        if not os.path.exists(kitti_dataset_root_dir):
            raise Exception(f"Check dataset directory: {os.path.abspath(kitti_dataset_root_dir)}")

        for num in phase_set:
            directory = next(os.walk(f"{kitti_dataset_root_dir}/{num:0>2d}/{kitti_pc_data_directory_name}"))
            self.list_files.extend(
                map(
                    lambda x: f"{kitti_dataset_root_dir}"
                              f"/{num:0>2d}/{kitti_pc_data_directory_name}/" + x,
                    directory[2],
                )
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.labels:
            lidarseg_labels_filename = re.sub(
                kitti_pc_data_file_suffix, kitti_pc_label_file_suffix,
                re.sub(kitti_pc_data_directory_name, self.label_directory, lidar_file)
            )
            points_labels = (
...
def make_data_loader_semkitti(config, phase, num_threads=0):
    """
