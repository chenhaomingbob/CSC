...
from downstream.panoptic_segmentation.eval_miou import fast_hist_crop
...
class LightningDownstreamCylinder3D(pl.LightningModule):
    def __init__(self, model, config):
        ...
        self.panoptic_evaluator = PanopticEval(num_classes,
                                               None,
                                               [self.ignore_label],  #
    ...
    def training_step(self, batch, batch_idx):
        ...
        # else:
        #     self.model.train()
        batch['batch_size'] = len(batch['pc_xyz'])
        output_dict = self(batch)
        total_loss, loss_dict = self.get_training_loss(output_dict, batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        batch['batch_size'] = len(batch['pc_xyz'])
        output_dict = self(batch)
        pred_dict = self.post_processing(output_dict, batch)
    ...
    def on_validation_epoch_end(self, ) -> None:
        ...
        if self.trainer.num_devices > 1:
            ...
            for i in range(world_size):
                part_file = os.path.join('./tmpdir', 'result_part_{}_{}.pkl'.format(i, world_size))
