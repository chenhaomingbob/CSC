...
class Res16UNetBase(ResNetBase):
    BLOCK = None  # BasicBlock
    PLANES = (32, 64, 128, 256, 256, 256, 256, 256)
    DILATIONS = (1, 1, 1, 1, 1, 1, 1, 1)
    LAYERS = (2, 2, 2, 2, 2, 2, 2, 2)  # (2, 3, 4, 6, 2, 2, 2, 2)
    INIT_DIM = 32
    OUT_PIXEL_DIST = 1
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        if config.get('prototype_head', False):
            self.final_prototype = conv(self.PLANES[7], out_channels, kernel_size=1, stride=1, bias=True, D=D)
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'final_prototype'):
            out_prototype = self.final_prototype(out_feat)
            ...
            if self.normalize_feature:
                return SparseTensor(
                    ...
                    out_prototype.F / torch.norm(out.F, p=2, dim=1, keepdim=True),
                    coordinate_map_key=out.coordinate_map_key,
                    coordinate_manager=out.coordinate_manager,
                )
            else:
                return out, out_prototype
        else:
            if self.normalize_feature:
                return SparseTensor(
                    out.F / torch.norm(out.F, p=2, dim=1, keepdim=True),
