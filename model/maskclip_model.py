...
class MaskClipHead(nn.Module):
    ...
    def load_text_embeddings(self):
        ...
        self.text_embeddings[:, :] = loaded[:, :]  # (16,512)
    ...
    def forward(self, inputs):
        ...
        if self.vit:
            ...
            if isinstance(x, list) and len(x) == 2:
                x, cls_token = x
            if v is not None:
                feat = self.proj(v)  # [B,768,14,26] -> [B,512,14,26]
                image_local = self.image_mapping_local(v)  # [B,768,14,26] -> [B,512,14,26]
            else:
                feat = self.proj(x)
        ...
        output = self.cls_seg(feat)  # feat跟text_embedding
...
class VisionTransformer(nn.Module):
    ...
    def resize_pos_embed(pos_embed, input_shpae, pos_shape, mode):
        ...
        pos_embed_weight = resize(
            pos_embed_weight, size=input_shpae, align_corners=False, mode=mode,antialias=True)
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            x, q, k, v = layer(x,
                               self.return_qkv[i] or (i == len(self.layers) - 1 and self.skip_last_attn))
...
class maskClipFeatureExtractor(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = VisionTransformer()  # ViT
    ...
    def forward(self, img):
        x = self.encoder(img)  # img [48,3,224,416] -> [(48,768,14,26),(48,364,768),(48,364,768),(48,768,14,26)]
        feat, out = self.decoder(x)  # [24,512,14,26], [24,16,14,26] 每个patch的分类

        feat = resize(
            input=feat,
            size=self.img_size,  # [224,416]
            mode='bilinear',
            align_corners=True,
            antialias=True
        )  # [24,512,224,416]
        ...
        seg_pred = out.argmax(dim=1)  # [12,224,416]
        return feat, seg_pred  # [12,512,224,416], [12,224,416]
