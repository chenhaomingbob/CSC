...
def adapt_weights(architecture):
    ...
    if not os.path.exists(f"weights/{architecture}.pt"):
        logger.info(f"downloading pretrained weights {architecture} from {weights_url[architecture]}")
...
class DilationFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        if config["image_weights"] == "imagenet":
            ...
            print(f"Downloading model weights from {model_url} to {torch.hub.get_dir()}")
            self.encoder.load_state_dict(model_zoo.load_url(model_url))
    ...
    def forward(self, x):
        if self.preprocessing:
            x = self.preprocessing(x)  # 图像归一化
        x = self.decoder(self.encoder(x))  # [48,3,224,416] -> [48,2048,56,104] -> [48,64,224,416]
