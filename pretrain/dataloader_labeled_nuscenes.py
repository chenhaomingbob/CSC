...
def minkunet_collate_pair_fn(list_data):
    ...
    return {
        ...
        'lidar_tokens': lidar_token_list,
        'len_batch': len_batch,
        "evaluation_point_labels":  point_sem_labels,
        "evaluation_voxel_labels":  voxel_sem_labels,
    }


class NuScenesLabeledMatchDataset(Dataset):
    """
    ...
    def __getitem__(self, idx):
        ...
        ### semantic labels  ###
        ########################
        lidar_top_token = lidar_token['LIDAR_TOP']
        lidarseg_labels_filename = os.path.join(
            self.nusc.dataroot, self.nusc.get("lidarseg", lidar_top_token)["filename"]
        )
        ...
        voxel_sem_labels = points_labels[indexes]
        ...
        return (
            ...
            img_path_list,
            lidar_token,
            points_labels,  # point-wise semantic labels     (N_1,)
            voxel_sem_labels,  # voxel-wise semantic labels  (N_2,)
        )
...
VAL_PHASE = 'val'
TEST_PHASE = 'test'
from utils.transforms import make_transforms_clouds, make_transforms_asymmetrical_val, make_transforms_asymmetrical


def make_labeled_matched_nuscenes_dataloader(config):
    """
    ...
    dataset = NuScenesLabeledMatchDataset(
        phase=phase,
