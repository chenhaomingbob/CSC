...
class NuScenesMatchDataset(Dataset):
    ...
    def __init__(
        ...
        self.double_data = config['double_data'] if "double_data" in config else False
        print("self.double_data:" + str(self.double_data))
        #
    ...
    def create_list_of_scans(self, scene):
        ...
        if self.double_data:
            print("Pay attention! Using double data")
            self.list_keyframes.extend(list_data[:int(len(list_data) // 2)])
        # self.list_keyframes.extend(list_data)
