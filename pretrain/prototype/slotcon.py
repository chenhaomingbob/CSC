...
class SemanticGrouping(nn.Module):
    ...
    def forward(self, x):
        ####
        x_prev = x # (N,64)
        slots = self.slot_embed(torch.arange(0, self.num_slots, device=x.device)).unsqueeze(0).repeat(x.size(0), 1, 1) # (2640,256,256)
        dots = torch.einsum('nkd,nd->nd', F.normalize(slots, dim=2), F.normalize(x, dim=1)) # (N,K,C) (N,C) -> (NK)
        attn = (dots / self.temp).softmax(dim=1) + self.eps
        slots = torch.einsum('nd,nk-> nkd', x_prev, attn / attn.sum(dim=0, keepdim=True)) # 更新后的slots
        return slots, dots
        ####

        # x_prev = x # (N,64)
        # slots = self.slot_embed(torch.arange(0, self.num_slots, device=x.device)).unsqueeze(0).repeat(x.size(0), 1, 1) # (1798,32,64)
        # dots = torch.einsum('nkd,nd->nk', F.normalize(slots, dim=2), F.normalize(x, dim=1)) # (N,K,C) (N,C) -> (N,K) (1798,64)
